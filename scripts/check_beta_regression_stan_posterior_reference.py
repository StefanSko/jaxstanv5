...
# ]
# ///
"""Compare hierarchical Beta-regression logistic posterior summaries against Stan."""
...
class BetaRegressionStanConfig:
    """Configuration for hierarchical Beta-regression Stan posterior comparisons."""
...
@dataclass(frozen=True)
class BetaRegressionStanResult:
...
def _build_bound(data: Mapping[str, object]) -> BoundModel:
    ...
    class HierarchicalBetaRegressionStanReferenceModel:
        ...
        log_phi = Param(Normal(math.log(18.0), 0.5))
    ...
    return cast(BindableModel, HierarchicalBetaRegressionStanReferenceModel).bind(
        n_groups=n_groups,
        group_idx=group_idx,
        x=x,
        y=y,
...
def _run(config: BetaRegressionStanConfig) -> tuple[BetaRegressionStanResult, ...]:
    ...
    data_path = stan_root / "data" / "hierarchical_beta_regression_logistic_varying_slopes.json"
    model_path = stan_root / "models" / "hierarchical_beta_regression_logistic_varying_slopes.stan"
    parameters = ("alpha_pop", "beta_pop", "sigma_alpha", "sigma_beta", "log_phi")
...
def _parse_args() -> BetaRegressionStanConfig:
    parser = argparse.ArgumentParser(
        description="Compare hierarchical Beta-regression logistic posteriors against Stan."
    )
    ...
    return BetaRegressionStanConfig(
        seed=args.seed,
...
def _validate_config(config: BetaRegressionStanConfig) -> None:
