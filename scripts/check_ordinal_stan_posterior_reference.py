...
class OrdinalStanConfig:
    """Configuration for ordinal-logistic Stan posterior comparisons."""
...
@dataclass(frozen=True)
class OrdinalStanResult:
    """Printable result for one compared scalar parameter or projection."""
...
def _jaxstan_draw_result(result: SamplerResult) -> DrawResult:
...
def _stan_draw_result(fit: StanFit) -> DrawResult:
    column_names = tuple(fit.column_names)
    draws = jnp.asarray(fit.draws(inc_warmup=False, concat_chains=False))
    lower = _draw_column(draws, column_names, name="cutpoints[1]")
    upper = _draw_column(draws, column_names, name="cutpoints[2]")
    return DrawResult(
        samples={
            "beta": _draw_column(draws, column_names, name="beta"),
            "cutpoints[0]": lower,
            "cutpoints[1]": upper,
            "cutpoint_gap": upper - lower,
        }
...
def _run(config: OrdinalStanConfig) -> tuple[OrdinalStanResult, ...]:
    ...
    data_path = stan_root / "data" / "ordinal_logistic_regression.json"
    model_path = stan_root / "models" / "ordinal_logistic_regression.stan"
    ...
    jaxstan_draws = _jaxstan_draw_result(jaxstan_result)
    jaxstan_summaries = tuple(
        summarize_scalar_draws(jaxstan_draws.samples, parameter=parameter)
        for parameter in parameters
    )

    stan_model = _cmdstan_model(model_path)
    with tempfile.TemporaryDirectory(prefix="ordinal-logistic-stan-") as output_dir:
        ...
        stan_draws = _stan_draw_result(stan_fit)
    stan_summaries = tuple(
        summarize_scalar_draws(stan_draws.samples, parameter=parameter) for parameter in parameters
    )
...
def _parse_args() -> OrdinalStanConfig:
    parser = argparse.ArgumentParser(
        description="Compare ordinal-logistic posteriors against Stan."
    )
...
def _validate_config(config: OrdinalStanConfig) -> None:
