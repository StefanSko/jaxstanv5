...
# ]
# ///
"""Compare hierarchical Binomial logistic posterior summaries against Stan."""
...
class BinomialStanConfig:
    """Configuration for hierarchical Binomial Stan posterior comparisons."""
...
def _build_bound(data: Mapping[str, object]) -> BoundModel:
    ...
    @model
    class HierarchicalBinomialStanReferenceModel:
        """Hierarchical Binomial model matching the Stan fixture."""
    ...
    return cast(BindableModel, HierarchicalBinomialStanReferenceModel).bind(
        n_groups=n_groups,
        group_idx=group_idx,
        x=x,
        trials=trials,
...
def _run(config: BinomialStanConfig) -> tuple[BinomialStanResult, ...]:
    ...
    data_path = stan_root / "data" / "hierarchical_binomial_logistic_varying_slopes.json"
    model_path = stan_root / "models" / "hierarchical_binomial_logistic_varying_slopes.stan"
    ...
    with tempfile.TemporaryDirectory(prefix="hierarchical-binomial-stan-") as output_dir:
...
def _parse_args() -> BinomialStanConfig:
    parser = argparse.ArgumentParser(
        description="Compare hierarchical Binomial logistic posteriors against Stan."
    )
...
def _validate_config(config: BinomialStanConfig) -> None:
