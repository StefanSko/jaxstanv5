...
class NegativeBinomialStanConfig:
    """Configuration for hierarchical Negative-binomial Stan posterior comparisons."""
...
@dataclass(frozen=True)
class NegativeBinomialStanResult:
...
def _run(config: NegativeBinomialStanConfig) -> tuple[NegativeBinomialStanResult, ...]:
    ...
    data_path = stan_root / "data" / "hierarchical_negative_binomial_log_rate_varying_slopes.json"
    model_path = (
        stan_root / "models" / "hierarchical_negative_binomial_log_rate_varying_slopes.stan"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="hierarchical-negative-binomial-stan-") as output_dir:
...
def _parse_args() -> NegativeBinomialStanConfig:
    parser = argparse.ArgumentParser(
        description="Compare hierarchical Negative-binomial posteriors against Stan."
    )
    ...
    return NegativeBinomialStanConfig(
        seed=args.seed,
...
def _validate_config(config: NegativeBinomialStanConfig) -> None:
