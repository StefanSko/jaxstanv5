...
class PoissonStanConfig:
    """Configuration for hierarchical Poisson Stan posterior comparisons."""
...
def _run(config: PoissonStanConfig) -> tuple[PoissonStanResult, ...]:
    ...
    data_path = stan_root / "data" / "hierarchical_poisson_varying_slopes.json"
    model_path = stan_root / "models" / "hierarchical_poisson_varying_slopes.stan"
    ...
    with tempfile.TemporaryDirectory(prefix="hierarchical-poisson-stan-") as output_dir:
...
def _parse_args() -> PoissonStanConfig:
    parser = argparse.ArgumentParser(
        description="Compare hierarchical Poisson posteriors against Stan."
    )
    parser.add_argument("--seed", type=int, default=81_000)
    parser.add_argument("--num-chains", type=int, default=4)
    parser.add_argument("--num-warmup", type=int, default=600)
...
def _validate_config(config: PoissonStanConfig) -> None:
